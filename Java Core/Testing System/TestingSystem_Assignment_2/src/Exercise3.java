...
public class Exercise3 {
	public static void main(String[] args) {
		...
		DateTimeFormatter dateFormat = DateTimeFormatter.ofPattern("dd MMM, yyyy", new Locale("vi", "VN"));
		String dateString = dateFormat.format(exam1.createDate);
		System.out.println("Thong tin Exam1: \n id= " + exam1.id + "\n code= " + exam1.code + "\n title= " + exam1.title
				+ "\n category= " + exam1.category.name + "\n duration= " + exam1.duration + "\n creator= "
				+ exam1.creator.fullName + "\n createDate= " + dateString);
		...
		// Question 2
		System.out.println("Q2- Thong tin exam theo dinh dang nam-thang-ngay:");
		String pattern = "yyyy-MM-dd-HH-mm-ss";
		SimpleDateFormat simpleDateFormat = new SimpleDateFormat(pattern);
		for (Exam exam : exams) {
			// localDate + atatStartOfDay + default time zone + toInstant() = Date
			Date date = Date.from(exam.createDate.atStartOfDay(defaultZoneId).toInstant());
			String date1 = simpleDateFormat.format(date);
			System.out.println(exam.id + ": " + date1);
		}

		// Question 3
		System.out.println("Q3- Thong tin exam theo dinh dang chi in nam:");
		String pattern2 = "yyyy";
		SimpleDateFormat simpleDateFormat2 = new SimpleDateFormat(pattern2);
		for (Exam exam : exams) {
			Date date2 = Date.from(exam.createDate.atStartOfDay(defaultZoneId).toInstant());
			String date3 = simpleDateFormat2.format(date2);
			System.out.println(exam.id + ": " + date3);
		}

		// Question 4
		System.out.println("Q4- Thong tin exam theo dinh dang thang-nam:");
		String pattern3 = "yyyy-MM";
		SimpleDateFormat simpleDateFormat3 = new SimpleDateFormat(pattern3);
		for (Exam exam : exams) {
			Date date4 = Date.from(exam.createDate.atStartOfDay(defaultZoneId).toInstant());
			String date5 = simpleDateFormat3.format(date4);
			System.out.println(exam.id + ": " + date5);
		}
		
		// Question 5
		System.out.println("Thong tin exam theo dinh dang thang-ngay:");
		String pattern4 = "MM-dd";
		SimpleDateFormat simpleDateFormat4 = new SimpleDateFormat(pattern4);
		for (Exam exam : exams) {
			Date date6 = Date.from(exam.createDate.atStartOfDay(defaultZoneId).toInstant());
			String date7 = simpleDateFormat4.format(date6);
			System.out.println(exam.id + ": " + date7);
		}
		
		//String formatString = exam.createDate.format(DateTimeFormatter.ofPattern("MM-dd"));
